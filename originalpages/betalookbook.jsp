...
<!-- Tool tip for Lookbook by YES -->
<script async type="text/javascript">
	$(function() {
	$('.tooltip_book').tipsy({trigger: 'hover', gravity: 'n'});
	$('.tooltip_video').tipsy({trigger: 'hover', gravity: 'n',fade:true});
	 });
</script>
...
<div class="menu_pathway">
         ...
         <h1 class="nb">Lookbook</h1>
         <div class="content_holder">
         <c:set var="lbCount" value="0"/>
         <ul class="lookbook_gallery">
         <c:forEach items="${lbbanners}" var="lookbook">
         <!--<c:set var="banners" value="${lbbanners}"/>-->
         <%-- <input type="text" value="${lookbook.albumCoverImageURL}" ></input> --%>
         <c:set var="lbCount" value="${lbCount+1}"/>
         <c:set var="bannerid" value="${lookbook.key}"/>
         <c:set var="lbAlbumName" value="${lookbook.albumName}"/>
         <c:set var="videoCoverImageURL" value="${lookbook.videoCoverImage}"/>
         <c:set var="youTubeURL" value="${lookbook.youtubeCode}"/>
         <c:set var="lbAlbumNameL" value="${fn:toLowerCase(lbAlbumName)}"/>
         <%-- <c:set var="lbAlbumNameSm" value="${fn:replace(lbAlbumNameL,' ','')}"/> --%>
         <c:set var="lbAlbumNameSmall" value="${fn:replace(lbAlbumNameL,' ','_')}"/>
         <c:set var="imageLink" value="${lookbook.imageLink}"/>
         <c:set var="youTubeURLHash" value="1"/>
         <%-- <c:set var="videoLink" value="${lookbook.}"/> --%>
        <input type="hidden" value="${lookbook.deleted}"></input>
        <input type="hidden" value="${lbCount}"></input> 
       	<input type="hidden" value="${lbAlbumNameSmall}"></input>
       	<input type="hidden" value="${lbAlbumName}" id="albumName"></input>
       	<input type="hidden" value="${bannerid}" id="bannerid"></input>
		<input type="hidden" value="${lbCount}"></input> 
		<input type="hidden" value="${imageLink}" id="imageLink"></input>
		<input type="hidden" value="${lookbook.deleted}"></input>
		<input type="hidden" value="${youTubeURL}" id="youTubeURL"></input>
		<input type="hidden" value="${videoCoverImageURL}" id="videoCoverImageURL"></input>
         <c:if test="${lookbook.deleted eq false}" >	
         	 <c:if test="${lbCount%3!=0}">
        	 	<li>
             		<a>
						<img src= "${lookbook.albumCoverImageURL}" width="300" height="220"/>
						<div class="hover_state">
							
							<c:if test="${not empty lookbook.youtubeCode and lookbook.youtubeCode eq '#'}">
		                       	<div class="hover_state_holder hover_state_holder_s">
		                           <code class="book_icon tooltip_book" title="View Lookbook" id="bi_${bannerid}"></code>
                        	...
                        	<c:if test="${not empty lookbook.youtubeCode  and lookbook.youtubeCode ne '#'}">
                        		<div class="hover_state_holder ">
                        		   <code class="book_icon tooltip_book" title="View Lookbook" id="bi_${bannerid}"></code>
		                           <code class="video_icon tooltip_video" title="Watch Video" id="vi_${bannerid}"></code>
		                           <div class="clear_both"></div>
		                       	</div><!-- hover_state_holder -->
                        	</c:if>
                   		</div><!-- hover_state -->
