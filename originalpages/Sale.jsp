...
<meta name="KEYWORDS" content="${saleSize} shoes, ${saleSize} ${socialCatogery}'s shoes, ${saleSize}, ${socialCatogery}'s shoes">
<meta name="DESCRIPTION" content="${saleSize} shoes, ${saleSize} ${socialCatogery}'s shoes - Browse through a huge selection of ${saleSize} shoes at great prices and get free shipping and free returns at Solestruck.com.">
...
<link rel="canonical" href="/sale-shoes/">
<c:choose>
<c:when test="${pageno eq '1' }">
<link rel="next" href="/sale-shoes/page-${pageDTO.selectedPage+1}/"/>
</c:when>
<c:when test="${pageno eq pageDTO.availablePages }">
<link rel="prev" href="/sale-shoes/page-${pageDTO.selectedPage-1}/"/>
</c:when>
<c:otherwise>
<link rel="prev" href="/sale-shoes/page-${pageDTO.selectedPage-1}/"/>
<link rel="next" href="/sale-shoes/page-${pageDTO.selectedPage+1}/"/>
...
$(document).ready(function(){
	...
	function onMessage(message)
	{
		...
		$.each(msgobj,function(key,value){
			if(value!="0")
				{
					$('.incart_count_'+key).removeClass("dn");
					$('.incart_count_'+key).html("IN "+value+" <br> CARTS");
				}
...
<div class="menu_pathway">
	<ul>
               ...
               <%-- <c:choose>
				  ...
				  <c:otherwise>
				  	<li><a href="/sale-shoes/" >${youAreHere}</a></li>
				    <li><span>size-${param.attId}</span></li>
				  </c:otherwise>
				</c:choose> --%>
           </ul>
   <div class="clear_both"></div>
    ...
    <div class="sorry_no_items">
   	...
   	<c:if test="${currentPage eq 'SalePageBySize'}">
		...
		<input type="hidden" id="saleSize" value="${saleSize}" />
	...
	<div class="content_holder">
	    <div class="idp_shoe_name_holder"> 	
	    	<h1 class="nb all_heading">Sale</h1>
	...
	<input type="hidden" id="totavailpages" value="${pageDTO.availablePages}" >
		...
		<c:forEach items="${pageDTO.data}" var="newProMap">
			    ...
			    <c:set var="available" value="${newProMap.available}"></c:set>
         		<c:if test="${available eq true}"><c:set var="isAvailable" value="yes"></c:set></c:if>
           		<c:if test="${(shoeCount%3)!=1 && (shoeCount%3)!=0}"><c:set var="w_space" value="shoe_holder white_space"></c:set></c:if>
				...
				<a  style="text-decoration: none;"  href="/${fn:replace(vendorNameSmall,' ','-')}-${fn:replace(productNameSmall,' ','-')}-${fn:replace(colorSmall,' ','-')}/index.html"> 
					...
					<div class="shoe_discription_holder">
	                        	...
	                        	<c:choose>
	                        	<c:when test="${vendorNameSmall eq saleBrandNames }">
	                        	<fmt:formatNumber value="${sr:getRoundedValue(newProMap.salePrice-(newProMap.salePrice*discountprogram.discountPercentage)/100)}" var="SpSaleprice" type="currency" currencySymbol="$" />
	                        	<h3><strike>$${newProMap.retailPrice}</strike><strike>$${newProMap.salePrice}</strike><b class="incart">${SpSaleprice}</b></h3>
	                        	</c:when>
	                        	<c:otherwise>
					...
					<%-- <div class="shoe_discription_holder">
						<span class="circle_black incart_count_${newProMap.productId}">IN ${newProMap.inCart} <br> CARTS</span>
						...
						<fmt:formatNumber value="${newProMap.retailPrice}" var="price" type="currency" currencySymbol="$" />
							...
							<c:choose>
		                      <c:when test="${isAvailable eq 'yes'&& newProMap.salePrice > 0.00}">
		                     	 <h3><strike><c:out value="${price}"/></strike> <b><c:out value="${salePrice}"/></b></h3>
		      		  	   ...
		      		  	   <c:when test="${isAvailable eq 'no'}">
		      		  	    <h3>Out of Stock <b><c:out value="${newProMap.retailPrice}"/></b></h3>
						  	  </c:when>
						  	  <c:otherwise>
	...
	<input type="hidden" id="totavailpages" value="${pageDTO.availablePages}"/>
	<input type="hidden" id="paramAttID" value="${param.attId}"/>
	<input type="hidden" name="servername" id="servername" value=""></input>
	<input type="hidden" id="newsize" value="${fn:replace(saleSize,'.0','')}"/>
	<input type="hidden" name="sc" id="sc" value="${socialCatogery}"></input>
	<c:set var="newSize" value="${fn:replace(saleSize,'.0','')}"></c:set>
	
	<c:choose>
	
	      <c:when test="${socialCatogery eq null || socialCatogery eq '' }">
 		     <c:set var="salesizeurl" value="/sale-shoes"></c:set>
	      </c:when>
	      <c:when test="${newSize ne null && newSize ne '' && socialCatogery ne null }">
		     <c:set var="salesizeurl" value="/sale-${socialCatogery}s-shoes/size-${newSize}"></c:set>
	      </c:when>
	      <c:when test="${socialCatogery ne null}">
	    	  <c:set var="salesizeurl" value="/sale-${socialCatogery}s-shoes"></c:set>
	      </c:when>
			...
			<c:forEach var="i" begin="${lbegin}" end="${lend}" step="1" varStatus ="status">
				...
				<c:choose>
				      <c:when test="${i eq pageDTO.selectedPage}">
						    <a class="page_number_selected" href="${salesizeurl}/page-${i}/" id="pagecount_<c:out value="${i}" />" ><c:out value="${i}" /></a>
				      </c:when>
		 		     <c:otherwise >
			 		     <c:choose>
						  <c:when test="${param.attId eq null || param.attId eq ''}">
						    <a class="page_number" href="${salesizeurl}/page-${i}/" id="pagecount_<c:out value="${i}" />" ><c:out value="${i}" /></a>
						  </c:when>
						  <c:otherwise>
						    <a class="page_number" href="${salesizeurl}/page-${i}/" id="pagecount_<c:out value="${i}" />" ><c:out value="${i}" /></a>
						  </c:otherwise>
		 				...
		 				<c:choose>
						  <c:when test="${param.attId eq null || param.attId eq ''}">
						    <c:if test="${pageDTO.availablePages gt 10}" >
							 ...
							 <a class="next_page " id="next"   href="${salesizeurl}/page-${pageDTO.selectedPage+1}/"  ></a>
						  </c:when>
						  <c:otherwise>
						  	<c:if test="${pageDTO.availablePages gt 10}" >
						    	<a class="more_page" onclick="showNextMorePages()" title="More" >...</a>
	        ...
	        <li class="third">
	        	<div class="custom_dropdown" id="pagination_holder">
	               ...
	               <ul>
	                 <c:choose>
						  <c:when test="${param.attId eq null || param.attId eq ''}">
							 <li title="24" onclick="changeItemsPerPage('${salesizeurl}/',24)">24</li>
	                		 <li title="48" onclick="changeItemsPerPage('${salesizeurl}/',48)">48</li>
	                 		 <li title="96" onclick="changeItemsPerPage('${salesizeurl}/',96)">96</li>
						  </c:when>
						  <c:otherwise>
							 <li title="24" onclick="changeItemsPerPage('${salesizeurl}/',24)">24</li>
	                 		 <li title="48" onclick="changeItemsPerPage('${salesizeurl}/',48)" >48</li>
    ...
    <input type="hidden" id="attId" value="${param.attId}"></input>
    <input type="hidden" id="vendorId" value="${vendorId}"></input>
