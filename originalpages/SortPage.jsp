...
<meta name="KEYWORDS" content="${category}'s , ${category}'s shoes">
<meta name="DESCRIPTION" content="${category}'s , ${category}'s - Browse through a huge selection of ${category}'s shoes at great prices and get free shipping and free returns at Solestruck.com.">
...
<div id="wrapper">
  ...
  <!-- <div class="global_topbtm_scroll" style="display: block; ">
	...
	<div class="content_holder">
		...
		<c:choose>
		<c:when test="${attVal ne null}">
		<h1 class="nb all_heading">${youAreHere}'s ${attVal} Shoes</h1>
		</c:when>
		<c:otherwise>
	...
	<input type="hidden" id="sortPage" value="${sortPage}"/>
	    ...
	    <ul> 
			<li class="second">
			<a id="previous " <c:choose><c:when test="${pageDTO.selectedPage>1}">class="previous_page"</c:when><c:otherwise>class="empty_previous"</c:otherwise></c:choose> <c:if test="${pageDTO.selectedPage>1}">href="/${sortPage}/page-${pageDTO.selectedPage-1}/"</c:if> ></a>
